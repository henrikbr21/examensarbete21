            ...
            while (true) {
                ...
                if (input.equals("uci")) {
                    ...
                    for (int j = 0; j < 100; j++) {
                        ...
                        double result = engine.alphaBetaMax(board, 4, Double.NEGATIVE_INFINITY,
                                Double.POSITIVE_INFINITY, pv, 0, tpt.hash(board), false, false, new MeasurementData(),
                                0, false);
                    }
                    ...
                    while (st.hasMoreTokens()) {
                        ...
                        board.makeMove(Util.convertCoordToNum(move.substring(0, 2)),
                                Util.convertCoordToNum(move.substring(2)));
                    }
                    ...
                    if (nbrTokens < 2) {
                        ...
                        System.out.println(
                                "bestmove " + Util.convertNumToCoord(move.from) + Util.convertNumToCoord(move.to));
                    } else {
                        thread = new Thread(new Runnable() {
                            ...
                            public void run() {
                                try {
                                    ...
                                    double result = engine.search(board, "WHITE", 16, pv, false, new MeasurementData(),
                                            0);
                                    board.draw();
                                    GCStats stat = Util.dumpGCLogs();
                                    System.out.println(
                                            "Collection count: " + stat.count + " Collection time: " + stat.time);
                                    System.out.println("bestmove " + Util.convertNumToCoord(
                                            pv.get(pv.size() - 1).from) + Util.convertNumToCoord(
                                            pv.get(pv.size() - 1).to));
                        ...
                        thread = new Thread(new Runnable() {
                            ...
                            public void run() {
                                try {
                                    ...
                                    board.draw();
                                    double result = engine.search(board, "BLACK", 16, pv, false, new MeasurementData(),
                                            0);
                                    GCStats stat = Util.dumpGCLogs();
                                    System.out.println(
                                            "Collection count: " + stat.count + " Collection time: " + stat.time);
